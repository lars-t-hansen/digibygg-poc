...
static void publish_timer_callback(void *sensor_arg) {
    ...
    struct mgos_bme280_data data = {.temp=0.0, .press=0.0, .humid=0.0};
    mgos_bme280_read(sensor, &data);
    LOG(LL_INFO, ("Temperature: %f, Pressure: %f, Humidity: %f", data.temp, data.press, data.humid));
    time_t t = time(0);
    struct tm* timeinfo = localtime(&t);
    char timestamp[24];
    strftime(timestamp, sizeof (timestamp), "%FT%TZ", timeinfo);
    publish("{measurement: %Q, tags: {rom: %Q}, time: %Q, fields: {Temperature: %f, Pressure: %f, Humidity: %f}}", "bme280", "Kontor: Christer", timestamp, data.temp, data.press, data.humid);
}

enum mgos_app_init_result mgos_app_init(void) {
    struct mgos_bme280* sensor = mgos_bme280_i2c_create(119);
    mgos_set_timer(5000, true, publish_timer_callback, sensor);
